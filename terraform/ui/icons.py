...
external icon font/CDN, so this doesn't depend on network access. Same
icon set as ../scaffolder/icons.py, kept as a separate small copy since
each tool's static assets are self-contained (no shared static folder
between the two Flask apps). Each is a bare <svg>...</svg> string meant
to be rendered with Jinja's `|safe` filter.
"""
