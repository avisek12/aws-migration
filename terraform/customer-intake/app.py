"""
Customer Intake Chat — a self-service chat agent that asks a customer in
plain language what AWS landing zone account they need, then generates a
downloadable Terraform code folder for them to review and run themselves.
...
Run it with:
    ...
    python app.py
Then open http://127.0.0.1:5002/ yourself (no auto-open — same reasoning
...
# NOTE: temporarily reverted to the pre-LangGraph, per-session-object
# pattern to diagnose an OpenAIConnectionError — see app.langgraph_backup.py
# for the checkpointer-based version and restore it once isolated.
SESSIONS = {}      # session_id -> IntakeAgent
...
def chat():
    ...
    result = agent.user_says(message)
...
if __name__ == "__main__":
    ...
    print(f"\nCustomer Intake Chat running at {url}")
    ...
    # balancer — SESSIONS/DOWNLOADS/_rate_log are in-memory per-process,
    # so a request landing on a different process loses that user's
